...
function parseDayOfWeek(text) {
  ...
  if (
    ...
    lower.match(/monday\s*(through|to|-)\s*friday/)
  ) {
    return "1-5";
  }
...
function parseMonth(text) {
  ...
  if (foundMonths.length > 0) {
    foundMonths.sort(function sortEntries(a, b) {
      return a - b;
    });
...
var PATTERNS = [
  ...
  {
    regex:
      /^(?:every\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)$/i,
    handler: function handler(match) {
  ...
  {
    regex:
      /^(.+)\s+(?:every\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)$/i,
    handler: function handler(match) {
      var time = parseTime(match[1]);
  ...
  {
    regex:
      /^(?:every\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)(?:\s*,\s*|\s+and\s+|\s*&\s*)(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)(?:\s+at\s+(.+))?$/i,
    handler: function handler(match) {
  ...
  {
    regex:
      /^(?:every\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+(?:the\s+)?month)?$/i,
    handler: function handler(match) {
  ...
  {
    regex:
      /^(?:every\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+(?:the\s+)?month\s+)?at\s+(.+)$/i,
    handler: function handler(match) {
      var day = parseInt(match[1], 10);
      var time = parseTime(match[2]);
  ...
  {
    regex:
      /^on\s+the\s+(\d{1,2})(?:st|nd|rd|th)?(?:\s+(?:of\s+(?:the\s+)?month)?)?(?:\s+at\s+(.+))?$/i,
    handler: function handler(match) {
      var day = parseInt(match[1], 10);
      var time = match[2] ? parseTime(match[2]) : { hour: 0, minute: 0 };
      if (day < 1 || day > 31 || !time) {
  ...
  {
    regex:
      /^(?:every\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+and\s+(\d{1,2})(?:st|nd|rd|th)?(?:\s+(?:of\s+(?:the\s+)?month)?)?$/i,
    handler: function handler(match) {
  ...
  {
    regex:
      /^(?:every\s+)?(first|second|third|fourth|fifth|last|1st|2nd|3rd|4th|5th)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)(?:\s+of\s+(?:the\s+)?month)?$/i,
    handler: function handler(match) {
      var ord = ORDINALS[match[1].toLowerCase()];
      var dayNum = DAYS[match[2].toLowerCase()];
      if (ord === undefined || dayNum === undefined) {
        return null;
      }
      if (ord === "L") {
        return "0 0 * * " + dayNum + "L";
  ...
  {
    regex:
      /^(?:every\s+)?(first|second|third|fourth|fifth|last|1st|2nd|3rd|4th|5th)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)(?:\s+of\s+(?:the\s+)?month)?\s+at\s+(.+)$/i,
    handler: function handler(match) {
      var ord = ORDINALS[match[1].toLowerCase()];
      var dayNum = DAYS[match[2].toLowerCase()];
      var time = parseTime(match[3]);
      if (ord === undefined || dayNum === undefined || !time) {
        return null;
      }
      if (ord === "L") {
  ...
  {
    regex:
      /^(?:every\s+)?(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)$/i,
    handler: function handler(match) {
  ...
  {
    regex:
      /^(?:every\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+)?(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)$/i,
    handler: function handler(match) {
      var day = parseInt(match[1], 10);
  ...
  {
    regex:
      /^(?:every\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+)?(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)\s+at\s+(.+)$/i,
    handler: function handler(match) {
      var day = parseInt(match[1], 10);
      var monthNum = MONTHS[match[2].toLowerCase()];
      var time = parseTime(match[3]);
      if (day < 1 || day > 31 || monthNum === undefined || !time) {
        return null;
      }
  ...
  {
    regex:
      /^(?:every\s+)?(?:bi-?weekly|fortnightly|every\s+other\s+week|every\s+two\s+weeks|every\s+2\s+weeks)$/i,
    handler: function handler() {
  ...
  {
    regex:
      /^every\s+(\d+)\s*min(?:ute)?s?\s+(?:on\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)(?:s)?$/i,
    handler: function handler(match) {
      var mins = parseInt(match[1], 10);
      var dayNum = DAYS[match[2].toLowerCase()];
      if (mins < 1 || mins > 59 || dayNum === undefined) {
        return null;
      }
      return "*/" + mins + " * * * " + dayNum;
    },
  ...
  {
    regex:
      /^every\s+(\d+)\s*hours?\s+(?:on\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)(?:s)?$/i,
    handler: function handler(match) {
      var hours = parseInt(match[1], 10);
      var dayNum = DAYS[match[2].toLowerCase()];
      if (hours < 1 || hours > 23 || dayNum === undefined) {
        return null;
      }
  ...
  // === TIME RANGE PATTERNS (between X and Y) ===
  {
    ...
    handler: function handler(match) {
      ...
      if (mins < 1 || mins > 59 || !startTime || !endTime) {
        return null;
      }
...
function validateCron(expression) {
  ...
  for (var i = 0; i < Math.min(parts.length, 6); i++) {
    ...
    if (!/^[\d\*\-\/\,\?\#LW]+$/i.test(field)) {
      return {
        valid: false,
        error: "Invalid character in " + fieldName + " field",
      };
    }
      ...
      if (rangeParts.length === 2) {
        ...
        if (!isNaN(start) && !isNaN(end)) {
          ...
          if (start < range.min || end > range.max) {
            return {
              valid: false,
              error: "Range in " + fieldName + " is out of bounds",
            };
          }
        }
      }
...
function getExamplePhrases() {
  return [
    ...
    {
      phrase: "every 15 minutes between 8am and 6pm",
      expression: "*/15 8-18 * * *",
    },
  ];
}
